...
3. Start the FastAPI server (the SQLite database will initialize automatically on startup):
```bash
...
a) Edit the FAQ.txt file in the Backend folder. Ensure distinct Q&A blocks or case studies are separated by a double newline.
b) Regenerate the vector database by running the embedding script:
```bash
python vector_db.py
